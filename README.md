...
| Pop the player out to its own window | The **⧉** button in the player bar (keep it on a second screen while you browse — click it again, or close the window, to dock it back). Right-click the detached video for **Always on top** or **Hide title bar** (then drag the video itself to move it); `Esc` leaves its fullscreen. |
