...
- **Stream-switch protection**: starting another stream while something records pops a clear choice. For a recording that rides the stream you're watching: **stop the recording and switch**, **switch & keep recording over a new connection** (needs a multi-stream account), or **keep watching** (recording continues). For a separate-connection recording: **watch the recorded channel** (no new connection), **play anyway** (remembered for the session), or **cancel**. Whenever a recording ends — planned or not — a clear notice appears in the status bar (and on the video in fullscreen)
...
| Pause / skip / volume | ⏸, −10s / +30s, and the volume slider + 🔊 mute in the player bar (skips appear for seekable content) |
