...
- **In-player options menu** (the ⚙ button): pick the audio track (language) and subtitle track, adjust audio delay, force an aspect ratio (16:9, 4:3, stretch, ...), and set the network buffer/cache size — applied live and remembered. Changing the aspect ratio letterboxes **inside** the player: the mini player's video is pinned to a **constant height** — dragging the splitter, resizing the window, long titles or EPG cards never change its size
...
- **Persistent EPG cache**: the XMLTV guide is saved to disk per playlist, so after a restart the EPG shows immediately from the previous session (a cache younger than 6 hours skips the download entirely; if a download fails, stale cached data is used instead of nothing)
